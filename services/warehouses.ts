...
async function listWarehouses(env: EnvSource, activeOnly: boolean): Promise<WarehouseOption[]> {
  const [rows, operatorCounts] = await Promise.all([
    fetchWarehouseRows(env, activeOnly),
    fetchOperatorCounts(env)
  ]);
  return rows
...
export async function listActiveWarehouses(env: EnvSource = process.env): Promise<WarehouseOption[]> {
  const warehouses = await listWarehouses(env, true);
...
export async function listAdminWarehouses(env: EnvSource = process.env): Promise<WarehouseOption[]> {
  return listWarehouses(env, false);
}
...
async function nextWarehouseCode(name: string, env: EnvSource) {
  const existing = await listWarehouses(env, false);
