...
function readCompositionFromFields(formData: FormData) {
  ...
  return {
    ...
    mediaPosition: readOptionalString(formData, "composition_media_position") ?? String(existing?.mediaPosition ?? "59% 50%"),
    mobileMediaPosition: readOptionalString(formData, "composition_mobile_media_position") ?? String(existing?.mobileMediaPosition ?? "56% 42%"),
