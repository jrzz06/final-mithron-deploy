...
const ACCOUNT_DISABLED = "This account has been disabled. Contact your administrator.";
const ROLE_UNAVAILABLE = "Your account is not fully set up yet. Contact your administrator.";
...
const OAUTH_CODE_MESSAGES: Record<string, string> = {
  ...
  "auth/unauthorized-domain": "This site is not authorized for Google sign-in. Add your domain in Supabase Authentication settings.",
  "auth/operation-not-supported-in-this-environment": "Google sign-in is redirecting…"
};
...
export function mapAuthErrorForClient(error: unknown, fallback = GENERIC_SIGN_IN_ERROR) {
  ...
  if (lower.includes("too many") || lower.includes("rate limit") || lower.includes("locked")) {
...
export function mapAuthPageNotice(input: {
  ...
  if (input.logout_reason === "session_idle") {
    return { tone: "neutral" as const, message: "Your session expired. Please sign in again." };
  }
