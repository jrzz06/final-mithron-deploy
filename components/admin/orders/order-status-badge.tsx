...
function resolveStatusTone(status: string): StatusTone {
  ...
  if (/(assigned|warehouse|picked|packed|processing)/.test(normalized)) {
...
export function OrderStatusBadge({ status, compact = false, className = "" }: OrderStatusBadgeProps) {
  const label = humanStatus(status) || status.replaceAll("_", " ");
