...
export function ReportPageShell({ title, description, children }: { title: string; description?: string; children: ReactNode }) {
  return (
    <div className="grid gap-5">
      <div>
        <Link href="/admin/reports" className="text-sm font-medium text-[var(--platform-accent)] hover:text-[var(--platform-accent-strong)]">
          Back to reports
        </Link>
        <h2 className="mt-3 text-lg font-medium text-[var(--platform-text-primary)]">{title}</h2>
