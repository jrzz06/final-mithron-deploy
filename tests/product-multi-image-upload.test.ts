...
import { MAX_PRODUCT_IMAGE_COUNT } from "@/lib/product-image-limits";
...
describe("product multi-image upload helpers", () => {
  ...
  it("ships multi-image fields in admin and supplier forms", () => {
    ...
    expect(adminPage).toContain('encType="multipart/form-data"');
    expect(multiField).toContain('name="image_files"');
    expect(multiField).toContain("multiple");
    expect(multiField).toContain('name="gallery_urls"');
