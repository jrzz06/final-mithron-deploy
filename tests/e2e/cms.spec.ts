...
test.describe("Production CMS testing", () => {
  test("homepage CMS surfaces render on storefront", async ({ page }) => {
    ...
    await expect(page.getByTestId("home-customer-testimonials")).toBeVisible({ timeout: 25_000 });
  ...
  test("admin CMS editor loads for authenticated admin", async ({ page }) => {
    ...
    await expect(page.locator("[data-homepage-cms-editor]")).toBeVisible({ timeout: 25_000 });

    const heroNav = page.locator('[data-homepage-cms-nav-item="hero"]');
    if (await heroNav.isVisible()) {
      await heroNav.click();
      await expect(page.locator("[data-cms-media-field]").first()).toBeVisible({ timeout: 15_000 });
    }
  });
  ...
  test("admin CMS hero headline matches storefront hero copy", async ({ page }) => {
    ...
    const editorText = await page.locator("[data-homepage-cms-editor]").innerText();
